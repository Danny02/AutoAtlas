...
/**
 * Packs pages of images using the maximal rectangles bin packing algorithm by
 * Jukka Jylänki. A brute force binary search is used to pack into the smallest
 * bin possible.
 *
 * @author Nathan Sweet
...
public class MaxRectsPacker {
    ...
    private Page packPage(List<Rect> inputRects) {
        ...
        BinarySearch widthSearch = new BinarySearch(minWidth, settings.maxWidth, 15, settings.pot);
        BinarySearch heightSearch = new BinarySearch(minHeight, settings.maxHeight, 15, settings.pot);
     ...
     * packed.
     */
    private Page packAtSize(boolean fully, int width, int height, List<Rect> inputRects) {
        Page bestResult = null;
        for (int i = 0, n = methods.length; i < n; i++) {
            maxRects.init(width, height);
            Page result = maxRects.pack(inputRects, methods[i]);
    ...
    class MaxRects {
        ...
        private final List<Rect> usedRectangles = new ArrayList<>();
        ...
        public Page getResult() {
            ...
            result.outputRects = new ArrayList<>(usedRectangles);
